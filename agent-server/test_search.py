...
Tests each layer independently so you can pinpoint failures fast:
  ...
  3. OpenAI search-query generation
...
# ===================================================================
# TEST 3: OpenAI search-query generation
# ===================================================================
async def test_query_generation():
    """Verify OpenAI produces sensible search queries from a goal."""
    section("Test 3: OpenAI search-query generation")

    api_key = os.getenv("OPENAI_API_KEY")
    if not api_key:
        record_skip("openai_key_present", "OPENAI_API_KEY not set")
        return

    record("openai_key_present", True)
